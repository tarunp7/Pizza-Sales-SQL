...
select (orders_details.quantity * pizzas.price) as total_revenue 
...
select pizza_types.name, sum(orders_details.quantity) as quantity from pizza_types 
...
select hour(order_time) as hours, count(orders.order_id) as order_count from orders 
group by hours ;
